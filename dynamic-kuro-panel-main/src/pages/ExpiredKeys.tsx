...
import { Key, Copy, Eye, EyeOff, MoreHorizontal, Trash2, RefreshCw } from "lucide-react";
...
const ExpiredKeys = () => {
  ...
  const [keys, setKeys] = useState<ExpiredKey[]>([]);
  ...
  const fetchExpiredKeys = async () => {
    ...
    const { data: keysData, error } = await supabase
      ...
      .select("id, key_code, game_id, duration_hours, expires_at")
      .eq("status", "expired");
  ...
  const handleDeleteAll = async () => {
    ...
    if (error || data?.error) {
      toast({ title: "Failed to delete expired keys", variant: "destructive" });
    } else {
      toast({ title: "All expired keys deleted successfully" });
      fetchExpiredKeys();
    }
  };
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Expired Keys</h1>
              <p className="text-gray-400 text-sm">
                Keys that have passed their expiration date
              </p>
            </div>
          </div>
        ...
        <PurpleCard title={`⏰ Expired Keys (${keys.length})`}>
          {loading ? (
                ...
                {keys.map((key) => (
                  <div key={key.id} className="rounded-xl border border-border/30 bg-card/50 p-3 space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                      ...
                      <span>{key.expires_at ? format(new Date(key.expires_at), "MMM d, yyyy HH:mm") : "N/A"}</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-purple-700/30">
                      ...
                      <TableHead className="text-purple-300">Expired At</TableHead>
